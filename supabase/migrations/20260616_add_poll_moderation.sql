alter table public.polls
  drop constraint if exists polls_status_check;

alter table public.polls
  add constraint polls_status_check
  check (status in ('active', 'pending', 'approved', 'published', 'rejected', 'closed', 'archived'));

drop policy if exists "Public can read active polls" on public.polls;
create policy "Public can read live polls"
  on public.polls
  for select
  to public
  using (status in ('active', 'published', 'approved'));

drop policy if exists "Public can read poll options for active polls" on public.poll_options;
create policy "Public can read poll options for live polls"
  on public.poll_options
  for select
  to public
  using (
    exists (
      select 1
      from public.polls
      where polls.id = poll_options.poll_id
        and polls.status in ('active', 'published', 'approved')
    )
  );

drop policy if exists "Public can read active poll votes" on public.poll_votes;
create policy "Public can read live poll votes"
  on public.poll_votes
  for select
  to public
  using (
    exists (
      select 1
      from public.polls
      where polls.id = poll_votes.poll_id
        and polls.status in ('active', 'published', 'approved')
    )
  );

drop policy if exists "Users can cast their own poll vote" on public.poll_votes;
create policy "Users can cast their own live poll vote"
  on public.poll_votes
  for insert
  to authenticated
  with check (
    auth.uid() = user_id
    and exists (
      select 1
      from public.polls
      where polls.id = poll_votes.poll_id
        and polls.status in ('active', 'published', 'approved')
    )
  );

drop policy if exists "Public can read hearts for active polls" on public.poll_hearts;
create policy "Public can read hearts for live polls"
  on public.poll_hearts
  for select
  to public
  using (
    exists (
      select 1
      from public.polls
      where polls.id = poll_hearts.poll_id
        and polls.status in ('active', 'published', 'approved')
    )
  );

drop policy if exists "Users can heart active polls" on public.poll_hearts;
create policy "Users can heart live polls"
  on public.poll_hearts
  for insert
  to authenticated
  with check (
    auth.uid() = user_id
    and exists (
      select 1
      from public.polls
      where polls.id = poll_hearts.poll_id
        and polls.status in ('active', 'published', 'approved')
    )
  );

drop policy if exists "Public can read comments for active polls" on public.poll_comments;
create policy "Public can read comments for live polls"
  on public.poll_comments
  for select
  to public
  using (
    exists (
      select 1
      from public.polls
      where polls.id = poll_comments.poll_id
        and polls.status in ('active', 'published', 'approved')
    )
  );

drop policy if exists "Users can comment on active polls" on public.poll_comments;
create policy "Users can comment on live polls"
  on public.poll_comments
  for insert
  to authenticated
  with check (
    auth.uid() = user_id
    and length(trim(text)) > 0
    and exists (
      select 1
      from public.polls
      where polls.id = poll_comments.poll_id
        and polls.status in ('active', 'published', 'approved')
    )
  );

create table if not exists public.poll_reports (
  id uuid primary key default gen_random_uuid(),
  poll_id uuid not null references public.polls(id) on delete cascade,
  user_id uuid not null references auth.users(id) on delete cascade,
  reason text not null,
  created_at timestamptz not null default now(),
  constraint poll_reports_unique unique (poll_id, user_id)
);

create index if not exists poll_reports_poll_id_idx
  on public.poll_reports (poll_id, created_at desc);

create index if not exists poll_reports_user_id_idx
  on public.poll_reports (user_id, created_at desc);

alter table public.poll_reports enable row level security;

drop policy if exists "Users can report polls" on public.poll_reports;
create policy "Users can report polls"
  on public.poll_reports
  for insert
  to authenticated
  with check (auth.uid() = user_id);

drop policy if exists "Users can read their own poll reports" on public.poll_reports;
create policy "Users can read their own poll reports"
  on public.poll_reports
  for select
  to authenticated
  using (auth.uid() = user_id);
