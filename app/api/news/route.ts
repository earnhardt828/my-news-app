...
function logProviderRawCount(providerName: string, count: number) {
  console.log("PROVIDER_RAW_COUNT", {
    provider: providerName,
    count,
  });
}

function logProviderImageCount(providerName: string, count: number) {
  console.log("PROVIDER_IMAGE_COUNT", {
    provider: providerName,
    count,
  });
}

function logProviderRejectedReason(
  providerName: string,
  reason: string,
  details: Record<string, unknown>
) {
  console.log("PROVIDER_REJECTED_REASON", {
    provider: providerName,
    reason,
    ...details,
  });
...
async function fetchGNewsArticles(params: ProviderFetchParams): Promise<ProviderResponse> {
  ...
  console.log("GNEWS_FETCH_STARTED", {
    requestCount: requests.length,
    query: effectiveQuery,
  });
  ...
  const responses = await Promise.allSettled(
    requests.map(async ({ url, category }) => {
      ...
      logProviderRawCount("GNews", (data.articles ?? []).length);
  ...
  logProviderRawCount("GNews", normalizedArticles.length);
  ...
  return {
    ...
    debug: {
      ...
      bodyPreview: normalizedArticles[0]
        ? {
            title: normalizedArticles[0].title,
            source: normalizedArticles[0].source,
            imageUrl: normalizedArticles[0].imageUrl,
          }
        : null,
...
async function fetchCurrentsArticles(params: ProviderFetchParams): Promise<ProviderResponse> {
  ...
  console.log("CURRENTS_FETCH_STARTED", { query: searchTerms });
...
async function fetchGuardianArticles(params: ProviderFetchParams): Promise<ProviderResponse> {
  ...
  console.log("GUARDIAN_FETCH_STARTED", { query: effectiveQuery });
...
async function collectArticles(params: ProviderFetchParams): Promise<NewsRouteResponse> {
  ...
  const providerDebug = providerResponses.reduce<NonNullable<NewsRouteResponse["providerDebug"]>>(
    (accumulator, result, index) => {
      ...
      if (providerName === "GNews") {
        accumulator.gnews = {
          ...
          bodyPreview: debug?.bodyPreview ?? null,
